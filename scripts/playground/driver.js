...
(async () => {
  const client = await MongoClient.connect('mongodb://127.0.0.1:27017/cm-prod', {});
  const np = client.db().collection('network_place');

  console.time('aggregate');
  const d = await np.aggregate([
    {
      '$match': {
        network: new ObjectId("17aba99d5d50f55f1dd0e58b"),
      },
    },
    {
      '$project': {
