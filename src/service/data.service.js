...
exports.resolveModelWhereClause = (resolver, model, where = {}) => {
  ...
  const $where = Object.entries(where).reduce((prev, [key, value]) => {
    ...
    if (field.isVirtual()) {
      ...
      const ids = Promise.all(ensureArray(value).map(v => resolver.match(modelRef).where(isPlainObject(v) ? v : { id: v }).many().then(docs => docs.map(doc => doc[virtualRef])))).then(results => _.uniq(_.flattenDeep(results)));
